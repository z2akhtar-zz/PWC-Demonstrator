 ...
 *-------------------------------- Arctic Core -----------------------------*/
/** @tagSettings DEFAULT_ARCHITECTURE=PPC|ZYNQ|MPC5607B|MPC5645S */
...
/* ----------------------------[private functions]---------------------------*/
#if defined(CFG_MPC5606B) || defined(CFG_MPC5645S)
/**
...
{
	Std_ReturnType res = E_OK;
	if((NULL == ConfigPtr) || !((group % ADC_NOF_GROUP_PER_CONTROLLER) < ConfigPtr->nbrOfGroups))
	{
...
#endif
  if ((E_OK == returnValue) && (ConfigPtr->groupConfigPtr[group%ADC_NOF_GROUP_PER_CONTROLLER].status->groupStatus == ADC_BUSY)){
      /** @req SWS_Adc_00216 */
      returnValue = E_NOT_OK;
  }
  return (returnValue);
...
Std_ReturnType Adc_CheckEnableHardwareTrigger (Adc_StateType adcState, const Adc_ConfigType *ConfigPtr, Adc_GroupType group){
    ...
    if( (ValidateInit(adcState, ADC_ENABLEHARDWARETRIGGER_ID) == E_NOT_OK) ||
            (ValidateGroup(ConfigPtr, group, ADC_ENABLEHARDWARETRIGGER_ID) == E_NOT_OK))
    {
        returnValue = E_NOT_OK;
    }
    ...
    for( int index = 0; index < ADC_NBR_OF_GROUPS; index++)
    {
...
Std_ReturnType Adc_CheckDisableHardwareTrigger (Adc_StateType adcState, const Adc_ConfigType *ConfigPtr, Adc_GroupType group){
    ...
    if( (ValidateInit(adcState, ADC_DISABLEHARDWARETRIGGER_ID) == E_NOT_OK) ||
            (ValidateGroup(ConfigPtr, group, ADC_DISABLEHARDWARETRIGGER_ID) == E_NOT_OK))
    {
        returnValue = E_NOT_OK;
    }
    else if( ADC_TRIGG_SRC_HW == ConfigPtr->groupConfigPtr[group].triggerSrc &&
