...
/* IMPROVMENT: The default index should be replace by proper value */
...
#define DET_VALIDATE(_exp,_api,_err ) \
        if( !(_exp) ) { \
          (void)Det_ReportError(MODULE_ID_NVM, 0, _api, _err); \
        }
...
#define DET_REPORTERROR(_module,_instance,_api,_err) (void)Det_ReportError(_module,_instance,_api,_err)
...
#if defined(USE_DEM)
#define DEM_REPORTERRORSTATUS(_err,_ev ) do { \
                                             if (_err != DEM_EVENT_ID_NULL) { Dem_ReportErrorStatus(_err, DEM_EVENT_STATUS_FAILED); } \
                                         } while (0)
#else
...
uint8 Nvm_WorkBuffer[ _ALIGN(NVM_MAX_BLOCK_LENGTH,VIRTUAL_PAGE_SIZE) + VIRTUAL_PAGE_SIZE ];
...
static boolean CheckMemIfJobFinished(void)
{
    ...
    if (!MemIfJobAdmin.JobFinished) {
        jobResult = MemIf_GetJobResult(DEFAULT_INDEX);
...
static boolean ReadAllInit(void)
{
    ...
    for (i = 0; i < ( NVM_NUM_OF_NVRAM_BLOCKS ); i++) {
        if ((BlockDescriptorList->SelectBlockForReadall)
                ...
                && ((!AdminBlockTable->BlockValid)            // IMPROVEMENT: Check if this is to be done like this
                || (!AdminBlockTable->BlockChanged))        // IMPROVEMENT: Check if this is to be done like this
#endif
                ) {
                            ...
                            boolean restoreFromRom )
{
    ...
    case BLOCK_STATE_MEMIF_REQ:
    {
        ...
        if( isWrite ) {
            ...
            if (admPtr->DataIndex < bPtr->NvBlockNum) {
                ...
                if (rv != E_OK) {
                    if ( MemIf_GetStatus(DEFAULT_INDEX) == MEMIF_IDLE ) {
    ...
    case BLOCK_STATE_MEMIF_PROCESS:
    {
        ...
        /* Check read */
        MemIf_JobResultType jobResult = MemIf_GetJobResult(DEFAULT_INDEX);
...
static void WriteAllCheckWriteResult(void)
{
    if (MemIfJobAdmin.JobStatus == E_OK) {
        // IMPROVEMENT: Check if redundant block shall be written NVM337
...
//    static uint32 crc32Left;
    ...
    case NVM_INVALIDATE_NV_BLOCK:
    {
        ...
        case NS_INIT:
            ...
            ret = MemIf_InvalidateBlock(nvmBlock->NvramDeviceId,
...
void NvM_JobEndNotification(void)
{
    MemIfJobAdmin.JobFinished = TRUE;
    MemIfJobAdmin.JobStatus = E_OK;
    MemIfJobAdmin.JobResult = MemIf_GetJobResult(DEFAULT_INDEX);
}
