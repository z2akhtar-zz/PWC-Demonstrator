...
DeviceSerialType *fileList[] = {
	...
	[0] = &CodeComposer_Device,		/* stdin  */
	[1] = &CodeComposer_Device,		/* stdout */
	[2] = &CodeComposer_Device,		/* stderr */
#endif
...
int _WRITE(  int fd, const void *buf, size_t nbytes) {
    ...
    if( fd > sizeof(fileList)/sizeof(fileList[0]) ) {
        abort();
    }

    if( fileList[fd]->write == NULL ) {
        nbytes = 0;
    } else {
