...
StatusType TerminateApplication(  ApplicationType applId, RestartType restartOption ) {
	...
	OS_VALIDATE( applId <= OS_APPLICATION_CNT, E_OS_ID);   /* @req 4.1.2/SWS_Os_00493 */
	OS_VALIDATE( restartOption <= NO_RESTART, E_OS_VALUE); /* @req 4.1.2/SWS_Os_00459 */
	OS_VALIDATE( !(Os_ApplGet(applId)->state == APPLICATION_TERMINATED), E_OS_STATE); /* @req 4.1.2/SWS_Os_00507 */
	...
	if( applId != OS_SYS_PTR->currApplId ) {
	    ...
	    /* IMPROVEMENT: MISSING OS494, OS287, OS535, OS536 */
	}
...
StatusType GetApplicationState(   ApplicationType applId,  ApplicationStateRefType value ) {
	...
	OS_VALIDATE(applId <= OS_APPLICATION_CNT, E_OS_ID);
