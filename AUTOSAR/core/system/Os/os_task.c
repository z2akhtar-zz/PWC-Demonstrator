...
#if defined(CFG_PPC) || defined(CFG_ARMV7_AR) || defined (CFG_ARM_CR4) || defined(CFG_RH850)
/**
...
void Os_Dispatch( uint32_t op ) {
	...
	if( pcbPtr != currPcbPtr ) {
	    ...
	    Os_ArchSetupContext(pcbPtr);
	    Os_ArchSwapContextTo(NULL, pcbPtr);
#elif defined(CFG_ARMV7_AR) || defined(CFG_ARM_CR4) || defined(CFG_RH850)
        Os_TaskMakeRunning(pcbPtr);
        PRETASKHOOK();
        /* Adjust stack pointer beyond the context area */
	    Os_StackSetup(pcbPtr);
...
void Os_Arc_GetTaskName(char *str, TaskType taskId) {
    char *name = Os_TaskGet(taskId)->constPtr->name;
