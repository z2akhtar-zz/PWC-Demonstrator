...
#define NO_DTC_DISABLED 0xFFFFFFFF
...
#if defined(DEM_USE_INDICATORS)
/* @req DEM499 */
#define INDICATOR_FAILED_DURING_FAILURE_CYCLE (uint8)(1<<0)
#define INDICATOR_PASSED_DURING_FAILURE_CYCLE (uint8)(1<<1)
#define INDICATOR_FAILED_DURING_HEALING_CYCLE (uint8)(1<<2)
#define INDICATOR_PASSED_DURING_HEALING_CYCLE (uint8)(1<<3)
...
{
    if(NULL != eventParam->DTCClassRef) {
        return ( 0 != (eventParam->DTCClassRef->DTCSeverity & dtcSeverityMask));
    } else {
        return FALSE;
    }
}
...
static boolean operationCycleIsStarted(Dem_OperationCycleIdType opCycle)
{
...
{
    return operationCycleIsStarted(eventParam->EventClass->ConfirmationCycleRef);
}
...
{
    if((eventParam->EventClass->ConfirmationCycleRef != DEM_OPERATION_CYCLE_ID_ENDMARK) && (eventStatusRecPtr->confirmationCounter >= eventParam->EventClass->ConfirmationCycleCounterThreshold) ) {
        return TRUE;
    } else {
        return FALSE;
    }
...
static boolean indicatorFailFulfilled(const Dem_EventParameterType *eventParam, const Dem_IndicatorAttributeType *indicatorAttribute)
{
    ...
    if( DEM_FAILURE_CYCLE_INDICATOR == indicatorAttribute->IndicatorFailureCycleSource ) {
        ...
        thresHold = eventParam->EventClass->ConfirmationCycleCounterThreshold;
        opCyc = eventParam->EventClass->ConfirmationCycleRef;
    }
    /* @req DEM501 */
 ...
 */
static boolean indicatorHealingCycleIsStarted(const Dem_EventParameterType *eventParam, const Dem_IndicatorAttributeType *indAttr)
{
        ...
        if( NULL != eventIdParamList[indx].EventClass->IndicatorAttribute  ) {
            ...
            while( !indAttrPtr->Arc_EOL) {
                ...
                if( DEM_FAILURE_CYCLE_INDICATOR == indAttrPtr->IndicatorFailureCycleSource ) {
                    ...
                    failCyc = eventIdParamList[indx].EventClass->ConfirmationCycleRef;
                }
    ...
    if( NULL != eventParam && (NULL != eventParam->EventClass->IndicatorAttribute)  ) {
        ...
        while( !indAttrPtr->Arc_EOL) {
            ...
            if( DEM_FAILURE_CYCLE_INDICATOR == indAttrPtr->IndicatorFailureCycleSource ) {
                ...
                failCyc = eventParam->EventClass->ConfirmationCycleRef;
            }
            if( (operationCycleId == failCyc) &&
...
static boolean handleIndicators(const Dem_EventParameterType *eventParam, EventStatusRecType *eventStatusRecPtr, Dem_EventStatusType eventStatus)
{
    ...
    if( NULL != eventParam->EventClass->IndicatorAttribute ) {
        ...
        while( !indAttrPtr->Arc_EOL) {
                ...
                switch(eventStatus) {
                    case DEM_EVENT_STATUS_FAILED:
                        if( (0 == (indicatorStatusBuffer[indAttrPtr->IndicatorBufferIndex].OpCycleStatus & INDICATOR_FAILED_DURING_FAILURE_CYCLE)) &&
                            ...
                            /* Should we reset healing counter here? */
                            indicatorStatusBuffer[indAttrPtr->IndicatorBufferIndex].HealingCounter = 0;
                            cntrChanged = TRUE;
                        }
            ...
            if( indicatorHealingCycleIsStarted(eventParam, indAttrPtr) ) {
                switch(eventStatus) {
                    case DEM_EVENT_STATUS_PASSED:
 ...
 */
static void updateEventStatusRec(const Dem_EventParameterType *eventParam, Dem_EventStatusType eventStatus, EventStatusRecType *eventStatusRec)
{
    ...
    if (eventStatusRecPtr != NULL) {
        ...
        /* Something wrong. All events should have been given a slot in preInit */
...
static boolean matchEventWithDtcFilter(const EventStatusRecType *eventRec)
{
    ...
    if ((dtcFilter.dtcStatusMask == DEM_DTC_STATUS_MASK_ALL) || (eventRec->eventStatusExtended & dtcFilter.dtcStatusMask)) {
        ...
        if ((eventRec->eventParamRef != NULL) &&
            checkDtcKind(dtcFilter.dtcKind, eventRec->eventParamRef) &&
            checkDtcOrigin(dtcFilter.dtcOrigin, eventRec->eventParamRef)) {
            ...
            if ((dtcFilter.filterWithSeverity == DEM_FILTER_WITH_SEVERITY_NO) ||
                ...
                if ((dtcFilter.filterForFaultDetectionCounter == DEM_FILTER_FOR_FDC_NO) ||
                    ((dtcFilter.filterForFaultDetectionCounter == DEM_FILTER_FOR_FDC_YES) && (checkDtcFaultDetectionCounter(eventRec->eventParamRef)))) {
                    dtcMatch = TRUE;
                }
            }
        }
...
static boolean eventDTCRecordDataUpdateDisabled(const Dem_EventParameterType *eventParam)
{
    boolean disabled = FALSE;
        if( (NO_DTC_DISABLED != DTCRecordDisabled.DTC) && /* There is a disabled DTC */
                ...
                (DTCRecordDisabled.DTC == eventParam->DTCClassRef->DTC) && /* The disabled DTC is the DTC of the event */
                                              ...
                                              uint16 bufferSize, Dem_EventIdType *eventToRemove)
{
    ...
    for( uint16 i = 0; i < bufferSize; i++ ) {
        if( DEM_EVENT_ID_NULL != eventBuffer[i].eventId ) {
            if(NO_DTC_DISABLED != DTCRecordDisabled.DTC) {
                lookupEventIdParameter(eventBuffer[i].eventId, &candidateEventParam);
                eventDataRemovalProhibited = eventDTCRecordDataUpdateDisabled(candidateEventParam);
            } else {
                eventDataRemovalProhibited = FALSE;
            }
 ...
 * @param dtcOriginFound - TURE if origin found otherwise FALSE
 * @return TRUE if any data deleted otherwise FALSE
 */
static boolean DeleteDTCData(const Dem_EventParameterType *eventParam, boolean *dtcOriginFound) {
...
static boolean isInEventMemory(const Dem_EventParameterType *eventParam)
{
    ...
    if( NULL != mem ) {
        for(uint16 i = 0; (i < memSize) && !found; i++) {
            if( eventParam->EventID == mem[i].eventId ) {
                found = TRUE;
            }
        }
    }
 ...
 */
static boolean getFreezeFrameRecord(Dem_EventIdType eventId,uint8 recordNumber, Dem_FreezeFrameClassType const *FFDataRecordClass,
    ...
    if (eventParam != NULL) {
        if (eventParam->EventClass->OperationCycleRef < DEM_OPERATION_CYCLE_ID_ENDMARK) {
            if (operationCycleStateList[eventParam->EventClass->OperationCycleRef] == DEM_CYCLE_STATE_START) {
                ...
                updateEventStatusRec(eventParam, eventStatus, &eventStatusLocal);
...
static Std_ReturnType handleEvent(Dem_EventIdType eventId, Dem_EventStatusType eventStatus)
{
    ...
    lookupEventIdParameter(eventId, &eventParam);
    if ( eventParam != NULL ) {
        if ( (eventParam->EventClass->OperationCycleRef < DEM_OPERATION_CYCLE_ID_ENDMARK) && operationCycleStateList[eventParam->EventClass->OperationCycleRef] == DEM_CYCLE_STATE_START) {
            if ((!((disableDtcSetting.settingDisabled) && (checkDtcGroup(disableDtcSetting.dtcGroup, eventParam)) && (checkDtcKind(disableDtcSetting.dtcKind, eventParam))))
                ...
                if (eventStatusLocal.errorStatusChanged || eventStatusLocal.extensionDataChanged) {
                    Std_ReturnType eventStoreStatus = storeEventEvtMem(eventParam, &eventStatusLocal); /** @req DEM184 */
                    boolean storeExtData = FALSE;
                    boolean storeFFData = FALSE;
                    boolean overrideOldExtData = FALSE;
...
#if defined(DEM_USE_INDICATORS) && defined(DEM_USE_MEMORY_FUNCTIONS)
                if(eventStatusLocal.indicatorDataChanged && isInEventMemory(eventParam)) {
                    storeEventIndicators(eventParam);
                    setIndicatorBlockChanged();
                }
#endif
            ...
            returnCode = E_NOT_OK; // Operation cycle not valid or not started
        }
    } else {
            ...
            case DEM_NO_PRE_DEBOUNCE:
                if (eventParam->EventClass->PreDebounceAlgorithmClass->PreDebounceAlgorithm.PreDebounceMonitorInternal != NULL) {
                    if (eventParam->EventClass->PreDebounceAlgorithmClass->PreDebounceAlgorithm.PreDebounceMonitorInternal->CallbackGetFDCntFnc != NULL) {
                        returnCode = eventParam->EventClass->PreDebounceAlgorithmClass->PreDebounceAlgorithm.PreDebounceMonitorInternal->CallbackGetFDCntFnc(counter); /** @req DEM204.None */ /** @req DEM264 */
                    }
                }
                break;
    ...
    if((evtStatusBuffer->eventStatusExtended & DEM_CONFIRMED_DTC) &&
       ...
       (!(evtStatusBuffer->eventStatusExtended & DEM_TEST_NOT_COMPLETED_THIS_OPERATION_CYCLE))) {
        ...
        if (agingRecFound) {
            ...
            if(agingRecLocal->agingCounter > evtStatusBuffer->eventParamRef->EventClass->AgingCycleCounterThreshold) {
...
static inline void initEventStatusBuffer(const Dem_EventParameterType *eventIdParamList)
{
    ...
    while( !eventIdParamList[indx].Arc_EOL ) {
        ...
        if(NULL != eventStatusRecPtr) {
            eventStatusRecPtr->eventId = eventIdParamList[indx].EventID;
            eventStatusRecPtr->eventParamRef = &eventIdParamList[indx];
            sint8 startUdsFdc = getDefaultUDSFdc(eventIdParamList[indx].EventID);
            eventStatusRecPtr->UDSFdc = startUdsFdc;
            eventStatusRecPtr->maxUDSFdc = startUdsFdc;
            eventStatusRecPtr->fdcInternal = 0;
        } else {
...
static void validateNvMBlockSizes(void)
{
    ...
    DEM_ASSERT( (0 == DEM_AGING_SECONDARY_NVM_BLOCK_HANDLE ) ||
                (DEM_AGING_SECONDARY_NVM_BLOCK_SIZE == sizeof(secMemAgingBuffer)));
#endif
    ...
    if (demState == DEM_INITIALIZED) {
        ...
        VALIDATE_RV(((filterWithSeverity == DEM_FILTER_WITH_SEVERITY_NO)
                    || ((filterWithSeverity == DEM_FILTER_WITH_SEVERITY_YES)
                        && (!(dtcSeverityMask & (Dem_DTCSeverityType)~(DEM_SEVERITY_MAINTENANCE_ONLY | DEM_SEVERITY_CHECK_AT_NEXT_HALT | DEM_SEVERITY_CHECK_IMMEDIATELY))))), DEM_SETDTCFILTER_ID, DEM_E_PARAM_DATA, DEM_WRONG_FILTER);
                        ...
                        if (checkDtcGroup(dtc, eventParam)) {
                            if( eventParam->EventClass->EventDestination == dtcOrigin ) {
                                if(FALSE == eventDTCRecordDataUpdateDisabled(eventParam)) {
                                    ...
                                    if( (NULL == eventParam->CallbackClearEventAllowed) ||  /* @req DEM514 */ /* @req DEM515 */
                                        (E_OK != eventParam->CallbackClearEventAllowed(&clearAllowed)) ||   /* @req DEM516 */
                                        clearAllowed) {

                                        boolean dtcOriginFound = FALSE;
                                        oldStatus = eventStatusBuffer[i].eventStatusExtended;
                                        dataDeleted = DeleteDTCData(eventParam, &dtcOriginFound);
                ...
                if (checkDtcOrigin(dtcOrigin, eventRec->eventParamRef)) {
                    if (lookupFreezeFrameDataRecNumParam(recordNumber, eventRec->eventParamRef, &FFDataRecordClass)) {
                        ...
                        if (*bufSize >= FFDataSize) {
                            ...
                            case DEM_DTC_ORIGIN_PRIMARY_MEMORY:
                            case DEM_DTC_ORIGIN_SECONDARY_MEMORY:
                                if (getFreezeFrameRecord(eventRec->eventId, recordNumber, FFDataRecordClass, dtcOrigin, destBuffer, bufSize, &FFDataSize)) {
...
Dem_ReturnGetSeverityOfDTCType Dem_GetSeverityOfDTC(uint32 DTC, Dem_DTCSeverityType* DTCSeverity)
{
    ...
    if( DEM_INITIALIZED == demState  ) {
        ...
        while( !DTCPtr->Arc_EOL && (DEM_GET_SEVERITYOFDTC_WRONG_DTC == ret) ) {
            if( DTC == DTCPtr->DTC ) {
                /* Dtc found */
        ...
        ret = DEM_GET_SEVERITYOFDTC_PENDING;
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GETSEVERITYOFDTC_ID, DEM_E_UNINIT);
    }
...
Dem_ReturnDisableDTCRecordUpdateType Dem_DisableDTCRecordUpdate(uint32 DTC, Dem_DTCOriginType DTCOrigin)
{
    ...
    if( DEM_INITIALIZED == demState  ) {
        if(NO_DTC_DISABLED == DTCRecordDisabled.DTC) {
            ...
            while( !eventIdParamPtr->Arc_EOL && (DEM_DISABLE_DTCRECUP_OK != ret)) {
                if( (NULL != eventIdParamPtr->DTCClassRef) && (eventIdParamPtr->DTCClassRef->DTC == DTC) ) {
        ...
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_DISABLEDTCRECORDUPDATE_ID, DEM_E_UNINIT);
        ret = DEM_DISABLE_DTCRECUP_PENDING;
    }
    return ret;
}
...
Std_ReturnType Dem_EnableDTCRecordUpdate(void)
{
    ...
    if( DEM_INITIALIZED == demState  ) {
        ...
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_ENABLEDTCRECORDUPDATE_ID, DEM_E_UNINIT);
    }
    return ret;
}
...
Std_ReturnType Dem_GetEventFreezeFrameData(Dem_EventIdType EventId, uint8 RecordNumber, boolean ReportTotalRecord, uint16 DataId, uint8* DestBuffer)
{
    ...
    if( DEM_INITIALIZED == demState  ) {
        ...
        if( (NULL != eventIdParamPtr) ) {
            /* Event has freeze frames configured */
            if(getFFRecData(eventIdParamPtr, RecordNumber, &ffRecordData, &recordToReport, &ffKind)) {
                /* And the record we are looking for was found */
                const Dem_FreezeFrameClassType *freezeFrameClass = NULL;
                const Dem_PidOrDidType **xidPtr;
                if(DEM_FREEZE_FRAME_NON_OBD == ffKind ) {
                ...
                uint16 destBufferIndex = 0;
                boolean done = FALSE;
                while( !(*xidPtr)->Arc_EOL && !done) {
                    if(DEM_FREEZE_FRAME_NON_OBD == ffKind ) {
    ...
    if( DEM_INITIALIZED == demState  ) {
        if( IS_VALID_EXT_DATA_RECORD(RecordNumber) || (ALL_EXTENDED_DATA_RECORDS == RecordNumber) ) {
            ...
            if( (NULL != eventIdParamPtr) && (NULL != eventIdParamPtr->ExtendedDataClassRef) ) {
                ...
                ret = E_OK;
                while( !done && (NULL != eventIdParamPtr->ExtendedDataClassRef->ExtendedDataRecordClassRef[extdataIndex])) {
                    readFailed = TRUE;
                    ...
                    if (lookupExtendedDataRecNumParam(extendedDataNumber, eventIdParamPtr, &extendedDataRecordClass, &posInExtData)) {
                        if( extendedDataRecordClass->UpdateRule != DEM_UPDATE_RECORD_VOLATILE ) {
                            if (lookupExtendedDataMem(EventId, &extData, eventIdParamPtr->EventClass->EventDestination)) {
                                // Yes all conditions met, copy the extended data record to destination buffer.
                                memcpy(&DestBuffer[destBufferIndex], &extData->data[posInExtData], extendedDataRecordClass->DataSize); /** @req DEM075 */
                                destBufferIndex += extendedDataRecordClass->DataSize;
                                readFailed = FALSE;
                            }
                        } else {
                            if( NULL != extendedDataRecordClass->CallbackGetExtDataRecord ) {
                                ...
                                getInternalElement(eventIdParamPtr, extendedDataRecordClass->InternalDataElement, &DestBuffer[destBufferIndex] );
                                destBufferIndex += extendedDataRecordClass->DataSize;
                                readFailed = FALSE;
                            }
                        }
...
Std_ReturnType Dem_ReadDataOfOBDFreezeFrame(uint8 PID, uint8 DataElementIndexOfPid, uint8* DestBuffer, uint8* BufSize)
{
    ...
    if (demState == DEM_INITIALIZED){
        ...
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_READDATAOFOBDFREEZEFRAME_ID, DEM_E_UNINIT);
    }
