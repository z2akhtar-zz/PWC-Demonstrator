...
#if (CANIF_PUBLIC_PN_SUPPORT == STD_ON) && FEATURE_NOT_SUPPORTED
    /* !req 4.0.3/CANIF750*/
    /* NOTE: Should this check be done before all pdu mode checks? */
...
void CanIf_RxIndication(Can_HwHandleType hrh, Can_IdType canId, uint8 canDlc,
    ...
    */
    ...
    if( !controllerFound ) {
        /* Hrh not found in configuration */
        /* @req 4.0.3/CANIF416 */
        DET_REPORT_ERROR(CANIF_RXINDICATION_ID, CANIF_E_PARAM_HRH);
        return;
    }
    /* @req 4.0.3/CANIF075 */
    /* @req 4.0.3/CANIF490 */
...
#if !defined(CFG_MPC5643L_FIX) // Mailbox filter does not seem to work on 5643L for some reason. Check all mailboxes for match
