...
#if defined(CFG_MPC5744P) || defined(CFG_MPC560X) || defined(CFG_MPC5645S) || defined(CFG_MPC5643L) || defined(CFG_SPC56XL70) || defined(CFG_MPC5777M)
...
void Port_RefreshPortDirection(void) {
    ...
    for (uint32 i = 0; Port_configPtr->padConfig[i].reg != PORT_INVALID_REG; i++) {
        if (!Port_configPtr->padConfig[i].pinDirectionChangeable) {
            /*lint -e{923} Ok, this cast is in the Freescale header file */
