...
static void enableNotification(Pwm_ChannelType Channel, Pwm_EdgeNotificationType Notification)
{
    ...
    // clear eventual pending notification
    flexHw->SUB[sub].STS.B.CMPF = enableFlags;
    /* Enable flags on this submodule */
    flexHw->SUB[sub].INTEN.B.CMPIE |= enableFlags;
...
static void SetDutyCycle(Pwm_ChannelType Channel, uint16 DutyCycle)
{
    ...
    leading_edge_position = (uint16) ((flexHw->SUB[sub].VAL[PwmRegIndex[pwm].periodRegIndex].R
                * (uint32) DutyCycle) / PWM_100_PERCENT );
    // clear eventual ldok flag
    flexHw->MCTRL.B.CLDOK = 1 << sub;
    ...
    if( PWM_X == pwm ) {
        ...
        if( PWM_0_PERCENT == DutyCycle ) {
            ...
            leading_edge_position = flexHw->SUB[sub].VAL[PwmRegIndex[pwm].periodRegIndex].R + 1;
        } else {
...
#if (PWM_NOTIFICATION_SUPPORTED==STD_ON)
    if( (PWM_100_PERCENT == DutyCycle) || (PWM_0_PERCENT == DutyCycle)) {
