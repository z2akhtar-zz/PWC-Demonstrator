 ...
 */
void GroupConversionComplete (Adc_GroupType group, const Adc_ConfigType *AdcConfigPtr, volatile struct ADC_tag *hwPtr);
/* ----------------------------[private variables]---------------------------*/
...
void GroupConversionComplete (Adc_GroupType group, const Adc_ConfigType *AdcConfigPtr, volatile struct ADC_tag *hwPtr)
{
    const Adc_GroupDefType *adcGroup = &AdcConfigPtr->groupConfigPtr[group%ADC_NOF_GROUP_PER_CONTROLLER];
...
void Group0ConversionComplete (uint32 unit)
{
	...
	const Adc_ConfigType *AdcConfigPtr = GetControllerConfigPtrFromHwUnitId(unit);
	...
	for (uint32 group = 0; group < AdcConfigPtr->nbrOfGroups; group++)
	{
	  if(AdcConfigPtr->groupConfigPtr[group].status->groupStatus != ADC_IDLE)
	  {
#if !defined (ADC_USES_DMA)
		/* Copy to result buffer */
		for(Adc_ChannelType index=0; index < AdcConfigPtr->groupConfigPtr[group].numberOfChannels; index++)
		{
            /* To get the layout of the buffer right, first all samples from one channel,
            than all samples for the next etc.*/
		    Adc_ValueGroupType *resultBufPtr = &AdcConfigPtr->groupConfigPtr[group].status->currResultBufPtr[index*AdcConfigPtr->groupConfigPtr[group].streamNumSamples];
#if defined(CFG_MPC5606S) || defined(CFG_MPC5645S)
		    uint32 chDataRegIndex = AdcConfigPtr->groupConfigPtr[group].channelList[index];
#else
		    uint32 chDataRegIndex = AdcConfigPtr->groupConfigPtr[group].channelList[index];
#endif
...
#endif
	    GroupConversionComplete((Adc_GroupType)group, AdcConfigPtr, hwPtr);
		break;
	  }
	}
}
