...
Std_ReturnType Fr_Internal_GetNewData(const Fr_ContainerType *Fr_Cfg, uint8 Fr_CtrlIdx, uint32 trigIdx, uint8* Fr_LSduPtr, uint8* Fr_LSduLengthPtr) {
    ...
    uint16 msgBufferIdx = Fr_Cfg->Fr_ConfigPtr->FrTrigConfig[Fr_CtrlIdx].FrMsgBufferCfg[trigIdx].FrMsgBufferIdx;
    Std_ReturnType retval;
...
Std_ReturnType Fr_Internal_IsSynchronous(const Fr_ContainerType *Fr_Cfg, uint8 Fr_CtrlIdx) {
    ...
    frccev = Fr_Cfg->hwPtr[Fr_CtrlIdx]->FRCCEV;
    errMode = (frccev & MASK_FRCCEV_ERRM) >> BIT_SHIFT_FRCCEV_ERRM;
    if (!((errMode == MASK_FRCCEV_ERRM_ACTIVE) || (errMode == MASK_FRCCEV_ERRM_PASSIVE))) {
        retval = E_NOT_OK;
    }
