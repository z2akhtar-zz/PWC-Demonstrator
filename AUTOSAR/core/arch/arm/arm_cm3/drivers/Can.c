...
#if defined(CFG_CAN_USE_SYMBOLIC_CANIF_CONTROLLER_ID)
        GET_CALLBACKS()->ControllerWakeup(canHwConfig->Can_Arc_CanIfControllerId);
#else
...
static void Can_ErrIsr( int unit ) {
    ...
    if(SET == CAN_GetITStatus(canHw, CAN_IT_BOF)){
        ...
        {
#if defined(CFG_CAN_USE_SYMBOLIC_CANIF_CONTROLLER_ID)
            GET_CALLBACKS()->ControllerBusOff(canHwConfig->Can_Arc_CanIfControllerId);
#else
            GET_CALLBACKS()->ControllerBusOff(unit);
#endif
        }
        Can_SetControllerMode(unit, CAN_T_STOP);/** @req 4.0.3/CAN272 *//** @req 4.1.2/SWS_Can_00272 */

        Can_AbortTx( canHw, canUnit );/** !req 4.0.3/CAN272 *//** !req 4.1.2/SWS_Can_00272 */
    ...
    if (err.R != 0)
    {
        ...
        {
#if defined(CFG_CAN_USE_SYMBOLIC_CANIF_CONTROLLER_ID)
            GET_CALLBACKS()->Arc_Error( canHwConfig->Can_Arc_CanIfControllerId, err );
#else
...
static void Can_RxIsr(int unit) {
    ...
    do {
        ...
        if (hohObj->CanObjectType == CAN_OBJECT_TYPE_RECEIVE)
        {
            ...
            {
                GET_CALLBACKS()->RxIndication(hohObj->CanObjectId, id, RxMessage.DLC, (uint8 *)&RxMessage.Data[0] ); // Next layer will copy
            }
...
void Can_Init( const Can_ConfigType *Config ) {
    ...
    for (int configId=0; configId < CAN_ARC_CTRL_CONFIG_CNT; configId++) {
        ...
        Can_ChangeBaudrate(ctlrId, canHwConfig->CanControllerDefaultBaudrate);
...
Std_ReturnType Can_ChangeBaudrate(uint8 Controller, const uint16 Baudrate)
{
    ...
    VALIDATE( (Controller < GET_CONTROLLER_CNT()), CAN_CHANGE_BAUD_RATE_SERVICE_ID, CAN_E_PARAM_CONTROLLER );
   ...
   if (baudratePtr == NULL)
   {
       return CAN_NOT_OK; // Baudrate not found
   }
...
Can_ReturnType Can_SetControllerMode( uint8 Controller, Can_StateTransitionType Transition ) {
    ...
    VALIDATE( (Controller < GET_CONTROLLER_CNT()), CAN_SETCONTROLLERMODE_SERVICE_ID, CAN_E_PARAM_CONTROLLER );
...
void Can_DisableControllerInterrupts( uint8 Controller )
{
    ...
    VALIDATE_NO_RV( (Controller < GET_CONTROLLER_CNT()), CAN_DISABLECONTROLLERINTERRUPTS_SERVICE_ID, CAN_E_PARAM_CONTROLLER );
...
void Can_EnableControllerInterrupts( uint8 Controller ) {
    ...
    VALIDATE_NO_RV( (Controller < GET_CONTROLLER_CNT()), CAN_ENABLECONTROLLERINTERRUPTS_SERVICE_ID, CAN_E_PARAM_CONTROLLER );
...
Can_ReturnType Can_Write( Can_HwHandleType Hth, const Can_PduType *PduInfo ) {
    ...
    VALIDATE( (Hth < NUM_OF_HTHS ), CAN_WRITE_SERVICE_ID, CAN_E_PARAM_HANDLE );
...
Std_ReturnType Can_CheckBaudrate(uint8 Controller, const uint16 Baudrate)
{
    ...
    return supportedBR ? CAN_OK : CAN_NOT_OK;
}
...
#else
            GET_CALLBACKS()->ControllerModeIndication(canHwConfig->CanControllerId, indicateMode);
#endif
        }
        if (clearPending) {
