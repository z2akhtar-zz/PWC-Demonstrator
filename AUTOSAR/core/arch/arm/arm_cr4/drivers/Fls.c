...
static Std_ReturnType Internal_EraseQueued();

void Fls_MainFunction(void)
{
	if (flsJobResult == MEMIF_JOB_PENDING) {
		if (FAPI_CHECK_FSM_READY_BUSY == Fapi_Status_FsmBusy) {
			//Module busy. Do nothing
		} else {
			uint32 stat = FAPI_GET_FSM_STATUS;
...
void ReportFlsError(void) {
	switch (pendingJob) {
	case FLS_JOB_ERASE:
		DEM_ERROR(FLS_E_ERASE_FAILED);
		break;
	case FLS_JOB_WRITE:
		DEM_ERROR(FLS_E_WRITE_FAILED);
		break;
	case FLS_JOB_READ:
		DEM_ERROR(FLS_E_READ_FAILED);
		break;
	case FLS_JOB_COMPARE:
		DEM_ERROR(FLS_E_COMPARE_FAILED);
		break;
	case FLS_JOB_NONE:
		break;
	}
...
void Fls_Init(const Fls_ConfigType* ConfigPtr) {
	...
	// @req SWS_Fls_00014
	FlsConfig = ConfigPtr;
	uint32_t clk = Mcu_Arc_GetPeripheralClock(PERIPHERAL_CLOCK_RTICLK);
...
Std_ReturnType Fls_Erase(Fls_AddressType TargetAddress, Fls_LengthType Length) {
	...
	VALIDATE_W_RV((ADDRESS_ALIGNED(ADDRESS32(TargetAddress),
				EEPROM_BASE, EEPROM_UPPER, PAGE_WIDE_SIZE)), FLS_ERASE_ID, FLS_E_PARAM_ADDRESS, E_NOT_OK);
	// @req SWS_Fls_00021
	VALIDATE_W_RV((Length > 0 && ADDRESS_ALIGNED(ADDRESS32(TargetAddress + Length),
					EEPROM_BASE, EEPROM_UPPER, PAGE_WIDE_SIZE)), FLS_ERASE_ID, FLS_E_PARAM_LENGTH, E_NOT_OK);
	// @req SWS_Fls_00327
...
Std_ReturnType Fls_Write(Fls_AddressType TargetAddress, const uint8* SourceAddressPtr,
	...
	Fapi_StatusType ret = Fapi_issueProgrammingCommand(ADDRESS32(jobAddress), jobPtr, jobLength, 0, 0, Fapi_DataOnly);
	if (ret != Fapi_Status_Success) return E_NOT_OK;
...
static Std_ReturnType Internal_WriteQueued()
{
	...
	Fapi_StatusType ret = Fapi_issueProgrammingCommand(ADDRESS32(queueAddress), queuePtr, jobLength, 0, 0, Fapi_DataOnly);
	if (ret != Fapi_Status_Success) {
		queuePtr = 0;
		return E_NOT_OK;
	}
