...
#if (SPI_SCALEABILITY == SPI_LEVEL_0) //then we do not have async at all
...
#if (SPI_SCALEABILITY == SPI_LEVEL_1) //Async only, so simulate sync by waiting
	Spi_AsyncTransmit(Sequence);
	while (Spi_GetSequenceResult(Sequence) == SPI_SEQ_PENDING);
#elif (SPI_SCALEABILITY == SPI_LEVEL_2) || (SPI_SCALEABILITY == SPI_LEVEL_0)
...
#if defined(USE_DEM)
				Dem_ReportErrorStatus(DemConf_DemEventParameter_EEP_E_COM_FAILURE, DEM_EVENT_STATUS_FAILED );
#endif
