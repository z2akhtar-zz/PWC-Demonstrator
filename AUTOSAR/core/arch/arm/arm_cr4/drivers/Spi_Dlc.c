...
static FUNC(uint16 , SPI_CODE) Spi_Dlc(Ram, Transmit)(uint32 HwUnit_u32);
static FUNC(uint16 , SPI_CODE) Spi_Dlc(Std, Transmit)(uint32 HwUnit_u32);
...
static FUNC(void, SPI_CODE) Spi_DlcTransmit(uint32 HwUnit_u32)
{
    ...
    if(DlcEv_pt->Guard_u8 == SPI_DLC_G_PENDING)
    {
        ...
        /* set baud rate */
        //SPI_SETVAL32(MUnit_pt->Reg_pt->Spi_EXT_PRESCALE1, DevCfg_pt->Spi_Baudrate);
...
(
    const Spi_DlcEventType* Dlc_Event_pt
)
{
